defmodule Mabeam.DemoAgent do
  ...
  def init(agent, config) do
    ...
    initial_state = if map_size(agent.state) > 0 do
  ...
  def handle_action(agent, :ping, _params) do
    ...
    # Emit a ping event
    {:ok, _event_id} = Mabeam.emit_event(:demo_ping, %{
      agent_id: agent.id,
  ...
  def handle_action(agent, :increment, %{"amount" => amount}) when is_integer(amount) do
    ...
    {:ok, _event_id} = Mabeam.emit_event(:demo_increment, %{
      agent_id: agent.id,
  ...
  def handle_action(agent, :add_message, %{"message" => message}) when is_binary(message) do
    ...
    {:ok, _event_id} = Mabeam.emit_event(:demo_message_added, %{
      agent_id: agent.id,
      message: new_message
    })
  ...
  def handle_action(agent, :clear_messages, _params) do
    ...
    {:ok, _event_id} = Mabeam.emit_event(:demo_messages_cleared, %{
      agent_id: agent.id,
  ...
  def handle_action(agent, :simulate_error, _params) do
    # Emit error event
    {:ok, _event_id} = Mabeam.emit_event(:demo_error, %{
      agent_id: agent.id,
  ...
  def handle_event(agent, event) do
    ...
    case event.type do
      :system_status ->
        ...
        {:ok, _event_id} = Mabeam.emit_event(:demo_status_response, %{
          agent_id: agent.id,
          status: :healthy,
  ...
  def terminate(agent, reason) do
    ...
    {:ok, _event_id} = Mabeam.emit_event(:demo_terminated, %{
      agent_id: agent.id,
      reason: reason,
