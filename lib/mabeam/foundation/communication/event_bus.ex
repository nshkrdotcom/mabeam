defmodule Mabeam.Foundation.Communication.EventBus do
  ...
  def handle_call({:subscribe_pattern, pattern, pid}, _from, state) do
    ...
    new_pattern_subscriptions = Map.update(
  ...
  def handle_info({:DOWN, ref, :process, _pid, _reason}, state) do
    ...
    new_subscriptions = Enum.reduce(state.subscriptions, %{}, fn {event_type, subscribers}, acc ->
      filtered_subscribers = Enum.reject(subscribers, fn {_pid, subscriber_ref} ->
        subscriber_ref == ref
      end)
    ...
    new_pattern_subscriptions = Enum.reduce(state.pattern_subscriptions, %{}, fn {pattern, subscribers}, acc ->
      filtered_subscribers = Enum.reject(subscribers, fn {_pid, subscriber_ref} ->
        subscriber_ref == ref
      end)
  ...
  defp match_parts([part | pattern_rest], [part | event_rest]), do: match_parts(pattern_rest, event_rest)
  
  # Handle "*" patterns
  defp match_parts(["*" | pattern_rest], event_parts) when length(event_parts) > 0 do
    case pattern_rest do
      [] -> true  # "*" at the end matches any remaining parts
      _ -> 
