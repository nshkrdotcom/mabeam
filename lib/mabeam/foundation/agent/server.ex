defmodule Mabeam.Foundation.Agent.Server do
  ...
  def init({agent, config}) do
    ...
    initialized_agent = case get_in(agent.metadata, [:module]) do
      nil ->
        agent
  ...
  def handle_cast({:message, message}, state) do
    ...
    final_state = if not state.processing do
  ...
  defp handle_agent_event(agent, event) do
    ...
    result = if agent_module && Code.ensure_loaded?(agent_module) && function_exported?(agent_module, :handle_event, 2) do
    ...
    # Return result from agent module or :ok
    result || :ok
  end
end