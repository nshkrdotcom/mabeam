defmodule Mabeam.Foundation.Agent.Lifecycle do
  ...
  def start_agent(agent_module, config \\ []) when is_atom(agent_module) do
    ...
    agent_id = case Keyword.get(config, :id) do
  ...
  def restart_agent(agent_id, config \\ []) when is_binary(agent_id) do
    with {:ok, agent} <- Registry.get_agent(agent_id) do
      ...
      case stop_agent(agent_id, :restart) do
        :ok ->
  ...
  """
  @spec update_lifecycle_state(binary(), Agent.lifecycle_state()) :: {:ok, Agent.t()} | {:error, term()}
  ...
  """
  @spec get_lifecycle_state(binary()) :: {:ok, Agent.lifecycle_state()} | {:error, :not_found}
